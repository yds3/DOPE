...
class LanguageEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.instr_act_encoder = BertOutAttention(config)
        self.instr_obj_encoder = BertOutAttention(config)
        self.instr_fuse_layernorm = BertLayerNorm(config.hidden_size, eps=1e-12)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
...
class GlocalTextPathCMT(BertPreTrainedModel):
    ...
    def forward(
        ...
        return_gmap_embeds=True,traj_objv2_fts=None, traj_objv2_lens=None,
        ...
        if traj_objv2_fts is not None:
            traj_obj_img_fts = traj_objv2_fts
            traj_vp_obj_lens = traj_objv2_lens
