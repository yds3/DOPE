...
# 这个 BaseAgent 类是一个用于生成和保存轨迹的基础类，它提供了基本的结构和方法，如初始化环境、生成轨迹、获取结果和测试代理。
...
# 这个方法 get_results 用于获取代理生成的轨迹结果。
...
# 这个 Seq2SeqAgent 类是一个用于处理序列到序列任务的代理，它可以处理自然语言指令并生成相应的轨迹。它包含了模型构建、训练、测试和保存/加载模型的方法。
